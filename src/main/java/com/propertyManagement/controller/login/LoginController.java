...
import com.propertyManagement.util.JWT;
...
//        }
        map.put("openId", "oclkH5_hsLJT4QIP0nBa6QsBXbT8");
        map.put("status", 1);
        return map;
    }
...
//    }
    ...
    public ResponseData phoneLogin(@RequestParam(value = "phone") String phone,
                                   @RequestParam(value = "password") String password){
        ResponseData responseData = new ResponseData();
        Account account = loginService.checkAccount(phone, password);
        if(account != null){
            //JWT加密生成token
            String token = JWT.sign(account, 60L* 1000L* 30L);
